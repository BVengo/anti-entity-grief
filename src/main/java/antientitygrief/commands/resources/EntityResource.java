...
public class EntityResource {
	public static RequiredArgumentBuilder<ServerCommandSource, RegistryEntry.Reference<EntityType<?>>> request(CommandRegistryAccess commandRegistryAccess) {
		return CommandManager.argument("entity", RegistryEntryReferenceArgumentType.registryEntry(commandRegistryAccess, RegistryKeys.ENTITY_TYPE)).suggests(SuggestionController.ENTITY_SUGGESTIONS);
	}

	public static String extract(CommandContext<ServerCommandSource> commandContext) throws CommandSyntaxException {
		return Utils.getEntityId(RegistryEntryReferenceArgumentType.getEntityType(commandContext, "entity").value());
	}
}